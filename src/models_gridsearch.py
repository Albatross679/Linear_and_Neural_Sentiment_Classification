...
def compute_lr_eval_metrics(model: LogisticRegressionClassifier, exs: List[SentimentExample]) -> dict:
    """Compute accuracy, precision, recall, F1 from a trained LR model on a set of examples."""
    tp = fp = fn = tn = 0
    for ex in exs:
        ...
        if pred == 1 and ex.label == 1:
            ...
            fp += 1
        elif pred == 0 and ex.label == 1:
            fn += 1
        else:
            tn += 1
    accuracy = (tp + tn) / len(exs) if len(exs) > 0 else 0.0
...
def train_logistic_regression(train_exs: List[SentimentExample], feat_extractor: FeatureExtractor,
    ...
    :param initial_lr: starting learning rate (default 0.5)
    :param lambda_reg: L2 regularization weight (default 0.1)
    :param min_count: feature frequency threshold (default 2)
    :param batch_size: mini-batch size for SGD (default 1)
    :param num_epochs: max training epochs (default 30)
    :param early_stopping_patience: 0=disabled; >0=stop if no dev acc improvement for N epochs
    ...
    # Feature frequency thresholding
    ...
    for epoch in range(num_epochs):
        ...
        for batch_start in range(0, len(indices), batch_size):
            ...
            for i in batch_indices:
                ...
                # Accumulate gradient: (y - prob) * x
                gradient_scalar = y - prob
                for idx, count in features.items():
                    grad_accum[idx] += gradient_scalar * count
                    active_indices.add(idx)

            # Apply averaged gradient with L2 regularization on active features
            for idx in active_indices:
                weights[idx] += learning_rate * (grad_accum[idx] / actual_batch_size - lambda_reg * weights[idx])
        ...
        if dev_exs is not None:
            ...
            for ex in dev_exs:
                ...
                if pred == ex.label:
                    dev_correct += 1
            dev_acc = dev_correct / len(dev_exs)
    ...
    # If early stopping was active but never triggered, use best weights if we tracked them
    if early_stopping_patience > 0 and best_weights is not None:
        weights = best_weights
...
def run_lr_grid_search(train_exs: List[SentimentExample], dev_exs: List[SentimentExample]) -> dict:
    """
    Random grid search over LR hyperparameters. Samples 20 of 162 combinations.
    Uses BetterFeatureExtractor with lr_decay=0.95. Returns best params dict.
    """
    search_space = {
        "lambda_reg": [0.1, 0.5, 0.8],
        "early_stopping_patience": [3, 4, 5],
        "min_count": [2, 3, 5],
        "batch_size": [16, 32],
        "initial_lr": [0.5, 0.3, 0.2],
    }
    ...
    sampled = rng.sample(all_combos, min(20, len(all_combos)))

    best_dev_acc = -1.0
    best_params = None

    print(f"\n=== LR Grid Search: testing {len(sampled)} of {len(all_combos)} combinations ===")
    for i, params in enumerate(sampled):
        feat_ext = BetterFeatureExtractor(Indexer())
        model, metrics = train_logistic_regression(
            train_exs, feat_ext, dev_exs=dev_exs,
        ...
        final_dev_acc = metrics["dev_acc"][-1] if metrics["dev_acc"] else 0.0
        print(f"  [{i+1}/{len(sampled)}] params={params} -> dev_acc={final_dev_acc:.4f}")
        if final_dev_acc > best_dev_acc:
            best_dev_acc = final_dev_acc
            best_params = params

    print(f"\n=== Best params (dev_acc={best_dev_acc:.4f}): {best_params} ===\n")
    return best_params
...
def train_linear_model(args, train_exs: List[SentimentExample], dev_exs: List[SentimentExample]) -> SentimentClassifier:
    ...
    if args.feats == "BETTER":
        ...
        best_params = run_lr_grid_search(train_exs, dev_exs)

        # Shared kwargs from grid search (lr_decay is NOT searched, fixed per config)
        shared_kwargs = dict(
        ...
        # Define 5 configs: (name, FeatureExtractorClass, lr_decay)
        configs = [
            ("Unigram", UnigramFeatureExtractor, 0.95),
            ("Bigram", BigramFeatureExtractor, 0.95),
            ("Better", BetterFeatureExtractor, 0.95),
            ("Unigram (Fixed LR)", UnigramFeatureExtractor, 1.0),
            ("Unigram (Aggressive)", UnigramFeatureExtractor, 0.8),
        ]
        ...
        for name, ExtractorClass, decay in configs:
            print(f"  Training {name} (lr_decay={decay})...")
            ext = ExtractorClass(Indexer())
            model, metrics = train_logistic_regression(
                train_exs, ext, dev_exs=dev_exs, lr_decay=decay, **shared_kwargs
            )
            all_metrics[name] = metrics

            # Print metrics for LaTeX table
            train_m = compute_lr_eval_metrics(model, train_exs)
            dev_m = compute_lr_eval_metrics(model, dev_exs)
            print(f"    {name}: Train Acc={train_m['accuracy']:.4f}, "
                  f"Dev Acc={dev_m['accuracy']:.4f}, "
                  f"Prec={dev_m['precision']:.4f}, "
                  f"Rec={dev_m['recall']:.4f}, "
                  f"F1={dev_m['f1']:.4f}")
